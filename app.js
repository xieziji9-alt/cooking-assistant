App({
  ...
  onLaunch() {
    ...
    // 自动扩充菜谱库，确保 300+ 且每种食材都有推荐
    try {
      const { generateRecipes } = require('./utils/autoRecipes.js')
      const auto = generateRecipes(this.globalData.ingredients, this.globalData.cookware, {
        minPerIngredient: 3,
        minTotal: 320,
        startId: 10000
      })
      // 合并：手工精选 + 自动生成（避免ID冲突均为 10000+）
      this.globalData.recipes = (this.globalData.recipes || []).concat(auto)
      console.log('菜谱总数：', this.globalData.recipes.length)
    } catch (e) {
